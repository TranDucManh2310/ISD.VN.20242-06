...
const ConfirmOrder = () => {
    ...
    const discount = 0 // nếu bạn có mã giảm giá thì thêm vào đây
    const totalPriceRaw = itemsPrice + shippingPrice + taxPrice - discount;
    const totalPrice = totalPriceRaw < 0 ? 0 : totalPriceRaw; // nếu tổng tiền âm thì trả về 0
    //const totalPrice = totalPriceRaw.toFixed(2); // nếu bạn cần hiển thị

    console.log({ itemsPrice, shippingPrice, taxPrice, totalPrice }); // ✅ kiểm tra đúng kiểu số
    ...
    const processToPayment = async () => {
        const data = {
            ...
            shippingPrice,
            taxPrice,
            totalPrice:Math.floor(totalPriceRaw)
        }
        ...
        const order = {
            ...
            totalPrice: data.totalPrice, // ⚠️ Phải có dòng này
  paymentInfo: {
    id: "TEMP_ID",
    status: "Chưa thanh toán"
        ...
        const res=await fetch('http://localhost:4000/api/v1/payment/create-payment-link',{
            method: 'POST', 
            headers: {
